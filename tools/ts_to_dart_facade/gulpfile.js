...
gulp.task("compile", () => {
  hasError = false;
  var tsResult = gulp
    .src(["lib/**/*.ts", "node_modules/typescript/lib/typescript.d.ts"])
...
gulp.task(
  "test.compile",
  gulp.series("compile", function (done) {
    if (hasError) {
      done();
      return;
    }
    return gulp
      .src(["test/**/*.ts", "node_modules/dart-style/dart-style.d.ts"], {
        base: ".",
      })
      .pipe(sourcemaps.init())
      .pipe(tsProject())
...
unitTests = gulp.series("test.compile", function (done) {
  ...
  return gulp.src("build/test/**/*.js").pipe(
    mocha({
...
gulp.task(
  "watch",
  gulp.series(unitTests, function () {
    failOnError = false;
...
gulp.task("copy", function () {
  return gulp
    .src(["lib/templates/*.txt"])
    .pipe(gulp.dest("build/lib/templates"));
});
