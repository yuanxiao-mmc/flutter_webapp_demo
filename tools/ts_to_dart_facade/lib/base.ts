...
export function isFunctionTypedefLikeInterface(
  ifDecl: ts.InterfaceDeclaration
): boolean {
  return (
    ifDecl.members &&
    ifDecl.members.length === 1 &&
...
export function isConstructor(n: ts.Node): n is Constructor {
  return (
    ts.isConstructorDeclaration(n) || ts.isConstructSignatureDeclaration(n)
  );
}
...
export function isCallableType(type: ts.TypeNode, tc: ts.TypeChecker): boolean {
  ...
  if (ts.isTypeReferenceNode(type)) {
    if (
      tc.getSignaturesOfType(tc.getTypeAtLocation(type), ts.SignatureKind.Call)
        .length > 0
    )
      return true;
  }
...
export function isThisParameter(param: ts.ParameterDeclaration): boolean {
  return (
    param.name && ts.isIdentifier(param.name) && param.name.text === "this"
  );
}
 ...
 */
export function filterThisParameter(
...
export function isPromise(type: ts.TypeNode): boolean {
  return (
    type && ts.isTypeReferenceNode(type) && ident(type.typeName) === "Promise"
  );
}
...
export function cloneNodeArray<T extends ts.Node>(
  src?: ts.NodeArray<T>
...
export function copyNodeArrayLocation(
  src: ts.TextRange,
  dest: ts.NodeArray<any>
) {
  dest.pos = src.pos;
  dest.end = src.end;
}
...
export function isInsideConstExpr(node: ts.Node): boolean {
  return isConstCall(
    <ts.CallExpression>getAncestor(node, ts.SyntaxKind.CallExpression)
  );
}
...
export function formatType(
  ...
  if (!comment || options.hideComment) {
    ...
    if (!result.error) {
      ...
      let expectedStubIndex =
        result.code.length - stubToMakeTypeValidStatement.length;
      if (
        result.code.lastIndexOf(stubToMakeTypeValidStatement) ===
        expectedStubIndex
      ) {
...
export class TranspilerBase {
  ...
  //3.0: 增加 optional 是否可为空、isExternal 是否 是 external
  visit(n: ts.Node, optional: boolean = false, isExternal: boolean = true) {
    this.transpiler.visit(n, optional, isExternal);
  }
  ...
  maybeWrapInCodeComment(
    { shouldWrap = true, newLine = false },
    emit: () => void
  ): void {
    if (shouldWrap) {
  ...
  //3.0: 增加 part 部分引用
  addImportPart(libraryUri: string) {
    if (!this.transpiler.importParts.includes(libraryUri)) {
      this.transpiler.importParts.push(libraryUri);
    }
  }
  ...
  resolveImportForSourceFile(
    ...
    const summary = this.addImport(
      this.transpiler.getDartFileName(fileName),
      identifier,
      true
    );
    if (summary.asPrefix) {
  ...
  visitNode(n: ts.Node, _: boolean = false, __: boolean): boolean {
  ...
  visitParameters(
    ...
    { namesOnly = false, extension = false }
  ) {
    this.emitNoSpace("(");
    ...
    for (; firstInitParamIdx < parameters.length; firstInitParamIdx++) {
      ...
      let isOpt =
        parameters[firstInitParamIdx].initializer ||
        parameters[firstInitParamIdx].questionToken ||
        parameters[firstInitParamIdx].dotDotDotToken;
      if (
        isOpt &&
        !ts.isObjectBindingPattern(parameters[firstInitParamIdx].name)
      ) {
        break;
      }
    }
    ...
    if (firstInitParamIdx !== parameters.length) {
      ...
      let positionalOptional = parameters.slice(
        firstInitParamIdx,
        parameters.length
      );
      //3.0: 可为空类型改造
      if (!namesOnly && extension) {
        this.emit("{");
      }
