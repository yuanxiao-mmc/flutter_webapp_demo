...
export class Transpiler {
  ...
  constructor(private options: TranspilerOptions = {}) {
    ...
    this.declarationTranspiler = new DeclarationTranspiler(
      this,
      this.fc,
      options.enforceUnderscoreConventions,
      options.promoteFunctionLikeMembers,
  ...
  /**
   * 输出模版文件
   * @param destination 生成路径
   */
  template(destination?: string): void {
    if (destination) {
      //3.0: 输出模版文件
      const fileNames = fs.readdirSync(__dirname + "/templates");
      for (const fileName of fileNames) {
        const outputFile = destination + "/" + fileName.replace(".txt", "");
        console.log("Output template file:", outputFile);
      ...
      console.log("destination is not define");
    }
  }
  ...
  transpile(fileNames: string[], destination?: string): void {
    ...
    if (this.options.basePath) {
      this.options.basePath = this.normalizeSlashes(
        path.resolve(this.options.basePath)
      );
    }
    ...
    const normalizedProgram = ts.createProgram(
      fileNames,
      this.getCompilerOptions(),
      host
    );
    const translatedResults = this.translateProgram(
      normalizedProgram,
      fileNames
    );
    ...
    for (const fileName of translatedResults.keys()) {
      if (destination) {
        const outputFile = this.getOutputPath(
          path.resolve(fileName),
          destination
        );
        console.log("Output facade file:", outputFile);
        mkdirP(path.dirname(outputFile));
  ...
  translateProgram(
    ...
    if (this.options.toJSON) {
      paths.set(
        ...
        JSON.stringify(ConvertedSyntaxKind, undefined, 2)
      );
    }
    return paths;
  }
  ...
  normalizeSourceFiles(
    ...
    if (program.getSyntacticDiagnostics().length > 0) {
      ...
      const error = new Error(
        `${first.start}: ${first.messageText} in ${first.file.fileName}`
      );
    ...
    const sourceFiles = program
      .getSourceFiles()
      .filter((f: ts.SourceFile) => fileNames.has(f.fileName));
    ...
    sourceFiles.forEach((f: ts.SourceFile) => {
      const normalizedFile = merge.normalizeSourceFile(
        ...
        this.fc,
        fileNames,
        this.options.renameConflictingTypes,
  ...
  private createCompilerHost(
    ...
    const defaultLibFileName = this.normalizeSlashes(
  ...
  public popContext() {
    if (this.outputStack.length === 0) {
      this.reportError(
        null,
        "Attempting to pop output stack when already empty"
      );
    }
  ...
  private translate(sourceFile: ts.SourceFile): string {
    ...
    this.imports.forEach((summary, name) => {
      const packageName = JSON.stringify(name);
      //3.0: 增加头文件导出 及 过滤
      if (summary.custom) {
        this.emit(`export ${packageName};\n`);
      }

      this.emit(`import ${packageName}`);
    ...
    //3.0: 增加 part 部分打印
    this.importParts.forEach((packageName) => {
      this.emit("\n");
      this.emit(`part \"${packageName}\";\n`);
    });
  ...
  getRelativeFileName(filePath?: string): string {
    ...
    if (filePath.indexOf(basePath) !== 0 && !filePath.match(/\.d\.ts$/)) {
      throw new Error(
        `Files must be located under base, got ${filePath} vs ${basePath}`
      );
    }
  ...
  isJsModuleFile(): boolean {
    ...
    return !("/" + this.currentFile.fileName).match(
  ...
  //3.0 增加 optional 是否可为空、isExternal 是否是 external
  visit(node: ts.Node, optional: boolean = false, isExternal: boolean = true) {
    ...
    for (let i = 0; i < this.transpilers.length; i++) {
      if (this.transpilers[i].visitNode(node, optional, isExternal)) return;
    }
...
export function getModuleResolver(compilerHost: ts.CompilerHost) {
  return (
    ...
    for (let mod of moduleNames) {
      let lookupRes = ts.resolveModuleName(
        mod,
        containingFile,
        COMPILER_OPTIONS,
        compilerHost
      ...
      lookupRes = ts.classicNameResolver(
        mod,
        containingFile,
        COMPILER_OPTIONS,
        compilerHost
...
class Output {
  ...
  emitType(s: string, comment: string) {
    this.emit(
      base.formatType(s, comment, { insideComment: this.insideCodeComment })
    );
  }
