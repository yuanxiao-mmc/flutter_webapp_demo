  ...
  visitNode(
    ...
    switch (node.kind) {
      case ts.SyntaxKind.SourceFile:
        ...
        this.emit(
          "// ignore_for_file: non_constant_identifier_names, unused_import, unnecessary_question_mark\n"
        );
        if (moduleName) {
      ...
      case ts.SyntaxKind.NamedImports:
        ...
        if (used.length === 0) {
          this.reportError(
            node,
            "internal error, used imports must not be empty"
          );
        }
        this.visitList(used);
        break;
      ...
      case ts.SyntaxKind.ExportDeclaration:
        ...
        if (exportDecl.moduleSpecifier) {
          this.emit(
            JSON.stringify(
              this.getExternalModuleReferenceExpr(exportDecl.moduleSpecifier)
            )
          );
        } else {
          this.reportError(
      ...
      case ts.SyntaxKind.ImportEqualsDeclaration:
        ...
        let fileName = this.getExternalModuleReferenceExpr(
          importEqDecl.moduleReference
        );
        this.getImportSummary(fileName).asPrefix = base.ident(
          importEqDecl.name
        );
        break;
