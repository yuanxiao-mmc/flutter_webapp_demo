...
import {
  FacadeConverter,
  identifierCanBeRenamed,
  isValidIdentifier,
} from "./facade_converter";
import { Transpiler } from "./main";
import {
  MergedMember,
  MergedParameter,
  MergedType,
  MergedTypeParameters,
} from "./merge";
...
type emitPropertyOptions = {
  ...
  isExternal: boolean;
  emitBody?: (
    declaration: ts.PropertyDeclaration | ts.PropertySignature
  ) => void;
};
...
export default class DeclarationTranspiler extends base.TranspilerBase {
  ...
  maybeEmitJsAnnotation(
    node: ts.Node,
    { suppressUnneededPaths }: { suppressUnneededPaths: boolean }
  ) {
   ...
   */
  maybeEmitFakeConstructors(decl: ts.Node, isExternal: boolean) {
    //3.0: 构造方法重构
    if (ts.isClassDeclaration(decl) && !isExternal) {
  ...
  //3.0: 增加 fromJson 方法
  visitJsonSerializable(decl: ts.Node) {
    if (ts.isClassDeclaration(decl)) {
      this.emit("\nfactory");
      this.fc.visitTypeName(decl.name);
      this.emit(".fromJson(Map<String, dynamic> json) => _$");
      this.fc.visitTypeName(decl.name);
      this.emitNoSpace("FromJson(json);\n");
    }
  }
  ...
  private notSimpleBagOfProperties(type: ts.Type): boolean {
    if (this.tc.getSignaturesOfType(type, ts.SignatureKind.Call).length > 0)
      return true;
    if (
      this.tc.getSignaturesOfType(type, ts.SignatureKind.Construct).length > 0
    )
      return true;
    if (type.symbol) {
      let declaration = type.symbol.declarations.find(
  ...
  hasOnlyProperties(
    ...
    outProperties: ts.PropertyDeclaration[]
  ): boolean {
    ...
    return this.hasOnlyPropertiesHelper(
  ...
  hasOnlyPropertiesHelper(
    properties: ts.NodeArray<ts.Declaration>,
    outProperties: ts.Declaration[]
  ): boolean {
    for (let i = 0; i < properties.length; ++i) {
      ...
      if (
        ...
        if (
          this.promoteFunctionLikeMembers &&
          isFunctionLikeProperty(node, this.tc)
        ) {
          return false;
        }
  ...
  //3.0: 增加是否是 isExternal
  visitClassBody(
    decl: base.ClassLike | ts.TypeLiteralNode,
    name: ts.Identifier,
    isExternal: boolean
  ) {
    ...
    this.visitMergingOverloads(decl.members, isExternal);
  ...
  visitMergingOverloads(
    ...
    members.forEach((node) => {
      ...
      switch (node.kind) {
        ...
        case ts.SyntaxKind.VariableDeclaration:
          {
            let propertyDecl = <
              ts.PropertyDeclaration | ts.VariableDeclaration
            >node;
            // We need to emit these as properties not fields.
            if (
              !this.promoteFunctionLikeMembers ||
              !isFunctionLikeProperty(propertyDecl, this.tc)
            ) {
            ...
            let funcDecl = <ts.FunctionLikeDeclaration>(
              ts.createNode(ts.SyntaxKind.MethodDeclaration)
            );
            ...
            switch (type.kind) {
              case ts.SyntaxKind.FunctionType:
                ...
                funcDecl.parameters = <ts.NodeArray<ts.ParameterDeclaration>>(
    ...
    orderedGroups.forEach((group: Array<ts.SignatureDeclaration>) => {
      ...
      if (group.length === 1) {
        this.visit(first, !isExternal, isExternal);
      ...
      group.forEach((fn: ts.Node) => {
        ...
        this.visit(fn, !isExternal, isExternal);
      ...
      for (let i = 1; i < group.length; ++i) {
        ...
        for (
          let j = mergedParams.length;
          j < signature.parameters.length;
          ++j
        ) {
      ...
      merged.parameters = ts.createNodeArray(
        mergedParams.map((p) => p.toParameterDeclaration())
      );
  ...
  visitNode(
    ...
    switch (node.kind) {
          ...
          if (paramDecl.dotDotDotToken) {
            ...
            if (type) {
              if (ts.isArrayTypeNode(type)) {
                ...
                console.error(
                  "Warning: falling back to dynamic for varArgs type: " +
                    type.getText()
                );
              }
            }
            ...
            for (
              let i = 1;
              i <= DeclarationTranspiler.NUM_FAKE_REST_PARAMETERS;
              ++i
            ) {
              if (i > 1) {
                this.emitNoSpace(",");
      ...
      case ts.SyntaxKind.SourceFile:
        ...
        this.visitMergingOverloads(sourceFile.statements, isExternal);
        if (this.containsPromises) {
      ...
      case ts.SyntaxKind.ModuleBlock:
        {
          ...
          this.visitMergingOverloads(block.statements, isExternal);
        }
        break;
      ...
      case ts.SyntaxKind.InterfaceDeclaration:
        {
          ...
          if (
            isInterface &&
            base.isFunctionTypedefLikeInterface(
              classDecl as ts.InterfaceDeclaration
            )
          ) {
          ...
          //3.0: 判断注解是否是 @DartObject， 这里没找到判断注解的好方式，硬写
          let isDartObject = classDecl.getText().indexOf("@DartObject") > -1;
          if (!isDartObject) {
            this.maybeEmitJsAnnotation(node, { suppressUnneededPaths: true });
          }
          if (
            isInterface ||
            this.hasModifierFlag(classDecl, ts.ModifierFlags.Abstract)
          ) {
            this.visitClassLike("abstract class", classDecl);
          } else if (isDartObject) {
            this.visitDartObjectClassLike(classDecl);
          } else {
  ...
  private visitProperty(
    ...
    //3.0：判断如果不是 extenral，则生成真实属性
    if (!isExternal) {
      this.maybeWrapInCodeComment(
        { shouldWrap: !canBeRenamed, newLine: true },
        () => {
          this.emitProperty({
            mode: emitPropertyMode.property,
            declaration: decl,
  ...
  //3.0: 生成 DartObject 对象实体，而不是 JS 映射对象
  private visitDartObjectClassLike(decl: base.ClassLike) {
    this.addImport("package:json_annotation/json_annotation.dart");
    //3.0: 加入 part 部分引用
    let fileName = decl.getSourceFile().fileName.split("/").pop().split(".")[0];
    this.addImportPart(`${fileName}.g.dart`);

    this.emit("@JsonSerializable()");
    return this.visitClassLikeHelper(
      "class",
      decl,
      decl.name,
  ...
  private visitClassLikeHelper(
    ...
    this.maybeEmitFakeConstructors(decl, isExternal);
    ...
    let synthesizePropertyParam = (param: ts.ParameterDeclaration) => {
      if (
        this.hasModifierFlag(param, ts.ModifierFlags.Public) ||
    ...
    (decl.members as ts.NodeArray<ts.Declaration>)
      ...
      .forEach((ctor) =>
        (<ts.ConstructorDeclaration>ctor).parameters.forEach(
          synthesizePropertyParam
        )
      );

    this.visitClassBody(decl, name, isExternal);

    if (!isExternal) {
      this.visitJsonSerializable(decl);
    }
    this.emit("}\n");
    if (this.promiseMembers.length) {
      const visitName = () => {
        this.visitClassLikeName(
          name,
          typeParameters,
          ts.createNodeArray(),
          false
  ...
  private emitProperty({
    ...
    if (mode === emitPropertyMode.getter) {
      ...
      this.visit(type);
      this.emit("v)");
    } else if (mode === emitPropertyMode.property) {
      this.visit(type);
      //3.0: 现只有可为空这一种情况
      this.emit("?");
      this.visitName(name);
  ...
  private emitMembersAsExtensions(
    ...
    visitClassName();
    this.emit("{");
    const mergedMembers = this.visitMergingOverloads(
      ts.createNodeArray(Array.from(methods)),
      true
    );
    this.emit("}\n");
    ...
    for (const merged of mergedMembers) {
      ...
      if (
        ...
        this.addImport("../utils.dart");
        //3.0: (暂不处理)这里需要变为 Future<T?> 返回可为空的 Future
        this.visit(declaration.type, true);
        this.visitName(declaration.name);
        this.visitParameters(declaration.parameters, {
          namesOnly: false,
          extension: true,
        });
        //3.0: 增加 async
        ...
        this.emit("var result = dartify(resoponse);\n");
        this.emit("return");
        let responseName = this.fc.generateDartTypeName(declaration.type);
        this.emit(/\<(.*?)\>/g.exec(responseName)[1]);
        this.emit(".fromJson(result);");
        this.emit("}\n");
      }
    }
    this.emit("}\n");
  ...
  private emitExtensionMethodBody({
    ...
    for (const arity of sortedArities) {
      if (arity < mergedDeclaration.parameters.length) {
        ...
        const suppliedParameters = mergedDeclaration.parameters.slice(
          0,
          firstOptionalIndex
        );
        const omittedParameters = mergedDeclaration.parameters.slice(
          firstOptionalIndex,
          mergedDeclaration.parameters.length
        );
        ...
        this.emit(") {");
        this.emit("var resoponse = await promiseToFuture(");
        this.emit(PRIVATE_CLASS_INSTANCE_IN_EXTENSIONS);
        this.emit(".");
        this.visitName(mergedDeclaration.name);
        this.visitParameters(ts.createNodeArray(suppliedParameters), {
          namesOnly: true,
        });
        this.emit("); }\n");
      } else {
        ...
        this.emit(".");
        this.visitName(mergedDeclaration.name);
        this.visitParameters(ts.createNodeArray(mergedDeclaration.parameters), {
          namesOnly: true,
        });
        this.emit(");\n");
      }
  ...
  private emitExtensionGetterBody(
    declaration: ts.PropertyDeclaration | ts.PropertySignature
  ) {
  ...
  private emitExtensionSetterBody(
    declaration: ts.PropertyDeclaration | ts.PropertySignature
  ) {
    this.emit(PRIVATE_CLASS_INSTANCE_IN_EXTENSIONS);
    this.emit(".");
    ...
    this.fc.emitPromisesAsFutures = true;
    this.emit(
      "(allowInterop((resolve, reject) { v.then(resolve, onError: reject); }));"
    );
  }
}
