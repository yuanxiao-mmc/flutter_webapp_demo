...
import {
  DART_LIBRARIES_FOR_BROWSER_TYPES,
  TS_TO_DART_TYPENAMES,
} from "./dart_libraries_for_browser_types";
...
export function isValidIdentifier(
  identifier: ts.DeclarationName | ts.PropertyName
): boolean {
  // Check if this identifier is being used as a class or type name
  const parent = identifier.parent;

  const isClassOrTypeName =
    parent &&
  ...
  if (
    isClassOrTypeName &&
    FacadeConverter.DART_BUILT_IN_IDENTIFIERS.has(text)
  ) {
    return false;
  }
...
export function identifierCanBeRenamed(
  identifier: ts.DeclarationName | ts.PropertyName
): boolean {
  const text = base.ident(identifier);
 ...
 * identifier name.
 */
export function fixupIdentifierName(
  node: ts.DeclarationName | ts.PropertyName
): string {
...
export class NameRewriter {
  ...
  private computeName(node: ts.NamedDeclaration): DartNameRecord {
    ...
    const sourceFile = <ts.SourceFile>(
      base.getAncestor(node, ts.SyntaxKind.SourceFile)
    );
    ...
    for (let i = 2; ; i++) {
      // must use for-loop because of eslint's no-shadow rule
  ...
  lookupName(node: ts.NamedDeclaration, context: ts.Node) {
    ...
    return this.fc.resolveImportForSourceFile(
      node.getSourceFile(),
...
function addInsideTypeArgument(
...
function removeResolvedTypeArguments(
  options: TypeDisplayOptions
): TypeDisplayOptions {
  let ret = cloneOptions(options);
...
export class FacadeConverter extends base.TranspilerBase {
  ...
  resolvePropertyTypes(tn: ts.TypeNode): Map<string, ts.PropertyDeclaration> {
    ...
    for (let sym of this.tc.getPropertiesOfType(t)) {
      let decl =
        sym.valueDeclaration || (sym.declarations && sym.declarations[0]);
      if (
        decl.kind !== ts.SyntaxKind.PropertyDeclaration &&
        decl.kind !== ts.SyntaxKind.PropertySignature
      ) {
        let msg =
          this.tc.getFullyQualifiedName(sym) +
  ...
  generateTypeList(
    ...
    return types
      .map((type) => {
        let typeName = that.generateDartTypeName(
          type,
          addInsideTypeArgument(options)
        );
        //3.0: (先不这么改)增加泛型也能返回空
  ...
  generateDartTypeName(
    ...
    if (ts.isTypeQueryNode(node)) {
      ...
      name = "dynamic";
      const objectTypeName = this.generateDartTypeName(
        node.objectType,
        options
      );
      const indexTypeName = this.generateDartTypeName(
        node.indexType,
        addInsideComment(options)
      );
      ...
      if (ts.isTypeAliasDeclaration(node.parent)) {
        ...
        if (parent.typeParameters && options.resolvedTypeArguments) {
          comment += "<";
          const resolvedParameters = parent.typeParameters.map((param) =>
            this.generateDartTypeName(
              options.resolvedTypeArguments.get(base.ident(param.name)),
              options
            )
          );
      ...
      comment =
        "Tuple of <" +
        this.generateTypeList(node.elementTypes, addInsideComment(options)) +
        ">";
      ...
      switch (type) {
        ...
        case "ReadonlyArray":
          name = this.generateDartName(
            node.typeName,
            setTypeArguments(options, node.typeArguments)
          );
          comment =
            "ReadonlyArray<" +
      ...
      this.reportError(
        node,
        "Unexpected TypeNode kind: " + ts.SyntaxKind[node.kind]
      );
    }
  ...
  visitTypeName(
    typeName: ts.EntityName | ts.PropertyName | ts.PropertyAccessExpression
  ) {
    ...
    const custom = this.lookupCustomDartTypeName(identifier, {
      insideComment: this.insideCodeComment,
      insideTypeArgument: this.insideTypeArgument,
    });
    if (custom) {
  ...
  getDeclarationOfReferencedType(
    ...
    const referenceSymbol = this.tc
      .getTypeAtLocation(type.typeName)
      .getSymbol();
    if (!referenceSymbol) {
      return undefined;
    }
  ...
  lookupCustomDartTypeName(
    ...
    if (symbol && symbol.flags & ts.SymbolFlags.TypeParameter) {
      ...
      if (
        options.resolvedTypeArguments &&
        options.resolvedTypeArguments.has(ident)
      ) {
        return {
          name: this.generateDartTypeName(
            options.resolvedTypeArguments.get(ident),
            removeResolvedTypeArguments(options)
          ),
        };
      }
      ...
      if (fileAndName) {
        ...
        if (fileSubs) {
          ...
          if (fileSubs.has(name)) {
            ...
            if (dartBrowserType) {
              this.addImport(
                DART_LIBRARIES_FOR_BROWSER_TYPES.get(name),
                subName,
                true
              );
            }
    ...
    if (declaration) {
      ...
      if (declaration.kind === ts.SyntaxKind.TypeAliasDeclaration) {
        ...
        if (supportedDeclaration) {
          return {
            name: this.maybeAddTypeArguments(
              this.nameRewriter.lookupName(declaration, identifier),
              options
            ),
            keep: true,
          };
        ...
        return {
          name: this.generateDartTypeName(
            alias.type,
            resolveTypeArguments(options, alias.typeParameters)
          ),
        };
      }
      ...
      if (
        ...
        if (
          ...
          base.isFunctionTypedefLikeInterface(
            <ts.InterfaceDeclaration>declaration
          ) &&
  ...
  findCommonType(type: ts.TypeNode, common: ts.TypeNode): ts.TypeNode {
    ...
    if (
      this.generateDartTypeName(common, { hideComment: true }) ===
    ...
    if (type.kind === ts.SyntaxKind.ArrayType) {
      ...
      array.elementType = this.toSimpleDartType([
        (common as ts.ArrayTypeNode).elementType,
    ...
    if (
      ...
      common.kind === ts.SyntaxKind.TypeReference
    ) {
    ...
    if (
      base.isCallableType(common, this.tc) &&
      base.isCallableType(type, this.tc)
    ) {
      // Fall back to a generic Function type if both types are Function.
      // TODO(jacobr): this is a problematic fallback.
      let fn = <ts.FunctionOrConstructorTypeNode>(
        ts.createNode(ts.SyntaxKind.FunctionType)
      );
      let parameter = <ts.ParameterDeclaration>(
        ts.createNode(ts.SyntaxKind.Parameter)
      );
      ...
      fn.parameters = ts.createNodeArray([
        parameter,
      ] as ReadonlyArray<ts.ParameterDeclaration>);
      return fn;
    }
  ...
  toTypeNode(type: ts.Type): ts.TypeNode {
    ...
    let referenceType = <ts.TypeReferenceNode>(
  ...
  createEntityName(symbol: ts.Symbol): ts.EntityName {
    ...
    identifier.escapedText = ts.escapeLeadingUnderscores(
  ...
  safeGetBaseTypes(type: ts.InterfaceType): ts.BaseType[] {
    ...
    const declaration = this.getTypeDeclarationOfSymbol(
      type.symbol
  ...
  commonSupertype(nodeA: ts.TypeNode, nodeB: ts.TypeNode): ts.TypeNode {
    ...
    if (
      nodeA.kind === ts.SyntaxKind.TypeReference &&
      nodeB.kind === ts.SyntaxKind.TypeReference
    ) {
      ...
      if (base.ident(refA.typeName) === base.ident(refB.typeName)) {
        let merge = <ts.TypeReferenceNode>(
          ts.createNode(ts.SyntaxKind.TypeReference)
        );
    ...
    return this.toTypeNode(
      this.getCommonSupertype(
        this.tc.getTypeAtLocation(nodeA),
        this.tc.getTypeAtLocation(nodeB)
      )
    );
  }
  ...
  private getLibFileAndName(
    ...
    if (
      symbol.flags &
      (ts.SymbolFlags.Class | ts.SymbolFlags.Function | ts.SymbolFlags.Variable)
    ) {
