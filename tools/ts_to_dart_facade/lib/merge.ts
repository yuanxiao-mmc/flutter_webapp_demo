    ...
    if (t) {
      ...
      switch (t.kind) {
        ...
        case ts.SyntaxKind.TypeReference:
          ...
          const decl = this.fc.getTypeDeclarationOfSymbol(
            this.fc.getSymbolAtLocation(typeRef.typeName)
          );
          if (decl && ts.isTypeAliasDeclaration(decl)) {
            ...
            if (!base.supportedTypeDeclaration(alias)) {
              if (typeRef.typeArguments) {
                console.log(
                  "Warning: typeReference with arguements not supported yet:" +
                    t.getText()
                );
              }
      ...
      this.types.set(
        this.fc.generateDartTypeName(t, { insideComment: true }),
        t
      );
    }
  }
...
export class MergedParameter {
  ...
  toParameterDeclaration(): ts.ParameterDeclaration {
    ...
    nameIdentifier.escapedText = ts.escapeLeadingUnderscores(
      Array.from(this.name).join("_")
    );
...
export class MergedTypeParameter {
  ...
  toTypeParameterDeclaration(): ts.TypeParameterDeclaration {
    let ret = <ts.TypeParameterDeclaration>(
      ts.createNode(ts.SyntaxKind.TypeParameter)
    );
    let nameIdentifier = <ts.Identifier>ts.createNode(ts.SyntaxKind.Identifier);
  ...
  merge(params: ts.NodeArray<ts.TypeParameterDeclaration>) {
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      if (this.mergedParameters.has(name)) {
        ...
        this.mergedParameters.set(
          name,
          new MergedTypeParameter(param, this.fc)
        );
      }
    }
  }
...
export function normalizeSourceFile(
   ...
   */
  function findConstructorInType(
  ...
  function getConstructedObjectType(
  ...
  ): ts.ObjectTypeDeclaration | undefined {
    const constructedTypeSymbol: ts.Symbol =
      constructor &&
    ...
    return constructedTypeSymbol.declarations.find((member: ts.TypeElement) => {
      ...
      return false;
    }) as ts.ObjectTypeDeclaration | undefined;
  }

  function mergeVariablesIntoClasses(
    n: ts.Node,
    classes: Map<string, base.ClassLike>
  ) {
      ...
      statement.declarationList.declarations.forEach(
        (variableDecl: ts.VariableDeclaration) => {
          if (ts.isIdentifier(variableDecl.name)) {
            ...
            let variableTypeDeclaration:
              | ts.TypeLiteralNode
              | ts.InterfaceDeclaration;
            if (ts.isTypeReferenceNode(variableType)) {
              variableTypeDeclaration = fc.getDeclarationOfReferencedType(
                variableType,
            ...
            const constructor: base.Constructor = findConstructorInType(
            ...
            if (constructor) {
              ...
              if (
                ts.isClassDeclaration(constructedType) ||
                ts.isInterfaceDeclaration(constructedType)
              ) {
                clazzTypeParameters = base.cloneNodeArray(
                  constructedType.typeParameters
                );
                clazzHeritageClauses = base.cloneNodeArray(
                  constructedType.heritageClauses
                );
              }
              ...
              if (ts.isClassDeclaration(constructedType)) {
                ...
                (clazz as base.ExtendedInterfaceDeclaration).constructedType =
                  constructedType;
              }
              ...
              if (renameConflictingTypes) {
                ...
                // combined.
                const variableSymbol = fc.getSymbolAtLocation(
                  variableDecl.name
                );
                  ...
                  for (const declaration of variableSymbol.getDeclarations()) {
                    if (
                      ...
                      declaration.name.escapedText =
                        ts.escapeLeadingUnderscores(name + "Type");
                    }
                  }
                }
                return;
            ...
            variableTypeDeclaration.members.forEach(
                ...
                switch (member.kind) {
                  ...
                  case ts.SyntaxKind.ConstructSignature:
                    {
                      ...
                      clonedConstructor.name = ts.getMutableClone(
                        variableDecl.name
                      ) as ts.PropertyName;
                      clonedConstructor.parent = existing;

                      const existingConstructIndex = members.findIndex(
                        base.isConstructor
                      );
                  ...
                  case ts.SyntaxKind.PropertySignature:
                    ...
                    if (!explicitStatic) {
                      ...
                      const existingDeclarations = findPropertyInHierarchy(
                        base.ident(member.name),
                        existing,
                        classes
                      );
                    ...
                    // currently handling.
                    if (
                      !findPropertyInClass(base.ident(member.name), existing)
                    ) {
                      if (!explicitStatic) {
                        addModifier(
                          member,
  ...
  function findPropertyInHierarchy(
    ...
    for (const clause of heritageClauses) {
      ...
      const declarationsInAncestors = findPropertyInHierarchy(
        propName,
        classes.get(name),
        classes
      );
      if (declarationsInAncestors.size) {
        declarationsInAncestors.forEach((decl) =>
          propertyDeclarations.add(decl)
        );
      }
    }
  ...
  function gatherClasses(n: ts.Node, classes: Map<string, base.ClassLike>) {
    if (
      ...
      if (classes.has(name)) {
        ...
        (classDecl.members as ts.NodeArray<ts.ClassElement>).forEach(
          (e: ts.ClassElement) => {
            // Small hack to get around NodeArrays being readonly
      ...
      ts.forEachChild(n, (child) =>
        mergeVariablesIntoClasses(child, moduleClasses)
      );
    } else if (ts.isModuleBlock(n)) {
